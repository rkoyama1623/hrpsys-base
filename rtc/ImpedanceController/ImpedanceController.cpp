...
void ImpedanceController::calcImpedanceOutput_DualArm() {
    ...
    if (DEBUGP) ifs.printp = true;
    ifs.calcInternalForce(ee_info);

    // for (std::map<std::string, ImpedanceParam>::iterator it = m_impedance_param.begin(); it != m_impedance_param.end(); it++) { // debug
    //     m_impedance_param_in[it->first] = m_impedance_param[it->first];
    // }
