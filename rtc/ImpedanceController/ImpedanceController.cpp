...
void ImpedanceController::calcImpedanceOutput_stopping(std::string limb_name, ImpedanceParam& param, hrp::BodyPtr m_robot) {
    /**
    ...
    while (it != m_impedance_param.end()) {
        ...
        if (param.is_active) {
            ...
            if ( param.transition_count > 0 ) {
                calcImpedanceOutput_stopping(it->first, param, m_robot);
            } else {
                // use impedance model
